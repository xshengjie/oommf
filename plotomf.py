...
if __name__==u'__main__':
    ...
    clip_xyz=np.array([[0,1],[0,1],[0e-9,100e-9]])   #array,x,y,z坐标范围的选择，
    clip_ms=np.array([[-1,1],[-1,1],[-1,1]])         #array,磁矩除以Ms后mx,my,mz磁矩范围的选择
   
    #画图相关，按需更改
    img_figsize=[1080,720]              #list,图片大小.
    ...
    img_view=np.array([-130,60])        #array,theta观察角度与Z轴夹角,phi观察角度与X轴夹角。可以为数组。
...
,img_display,img_ratio,img_view,img_axis,img_show,img_color,img_color_reverse,img_line_scale,img_save))
       
    ##########程序主体，不需要更改##################################
    #fpath=raw_input(u"请输入画图需要文档所在文件夹地址:")
    fpath=raw_input(u"Please enter the txt file folder address\n:")
    ffile=[i for i in os.listdir(fpath) if i.split('.')[-1]=='txt']#筛选后缀是txt的文件
